disp_avlbl = True
from os import environ

if 'DISPLAY' not in environ:
    disp_avlbl = False
    import matplotlib
    matplotlib.use('Agg')
import numpy as np
from time import time
import sys

sys.path.append('./')
from subprocess import call

from embedding.embeddingEntity import StaticGraphEmbedding


class Node2vec(StaticGraphEmbedding):
    def __init__(self, d, max_iter, walk_length, n_walks, window_ize, ret_p, inout_p):
        self._d = d
        self._max_iter = max_iter
        self._walkLength = walk_length
        self._numWalks = n_walks
        self._contextSize = window_ize
        self._return_p = ret_p
        self._inout_p = inout_p

        self._method_name = 'node2vec'
        self._X = None

    def get_method_name(self):
        return self._method_name

    def get_method_summary(self):
        return '%s_%d' % (self._method_name, self._d)

    def learn_embedding(self, graph=None, local_edge_file=None, is_weighted=False, no_python=False):
        args = ["./c_exe/node2vec"]
        if not graph and not local_edge_file:
            raise Exception('graph/edge file needed')
        if local_edge_file:
            graph = graph_util.loadGraphFromEdgeListTxt(local_edge_file)
        graph_util.saveGraphToEdgeListTxtn2v(graph, 'tempGraph.graph')
        args.append("-i:tempGraph.graph")
        args.append("-o:tempGraph.emb")
        args.append("-d:%d" % self._d)
        args.append("-l:%d" % self._walkLength)
        args.append("-r:%d" % self._numWalks)
        args.append("-k:%d" % self._contextSize)
        args.append("-e:%d" % self._max_iter)
        args.append("-p:%f" % self._return_p)
        args.append("-q:%f" % self._inout_p)
        args.append("-v")
        args.append("-dr")
        args.append("-w")
        start_time = time()
        try:
            call(args)
        except:
            raise Exception(
                './node2vec not found. Please compile snap, place node2vec in the path and grant executable permission')
        self._X = graph_util.loadEmbedding('tempGraph.emb')
        t2 = time()
        return self._X, (t2 - start_time)

    def get_embedding(self):
        return self._X

    def get_edge_weight(self, i, j):
        return np.dot(self._X[i, :], self._X[j, :])

    def get_reconstructed_adj(self, X=None, node_l=None):
        if X is not None:
            node_num = X.shape[0]
            self._X = X
        else:
            node_num = self._node_num
        adj_mtx_r = np.zeros((node_num, node_num))  # G_r is the reconstructed graph
        for v_i in range(node_num):
            for v_j in range(node_num):
                if v_i == v_j:
                    continue
                adj_mtx_r[v_i, v_j] = self.get_edge_weight(v_i, v_j)
        return adj_mtx_r
