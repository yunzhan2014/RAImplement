...
def random_walk_DiGraph_to_adj(di_graph, node_frac=0.1, n_walks_per_node=5, len_rw=2):
    t0 = time.time()
    n = di_graph.number_of_nodes()
    adj = np.zeros((n, n))
...
def addNodeAnomalies(di_graphs, p, k):
    ...
    for t in anomaly_time_steps:
        ...
        print
        'Nodes: %d, Edges: %d' % (di_graphs[t].number_of_nodes(), di_graphs[t].number_of_edges())
    return anomaly_time_steps


def save_Graph2Edge_ListTxt(graph, file_name):
